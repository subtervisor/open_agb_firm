...
/// @brief Returns and resets the accumulated stylus-vertical-drag delta in
/// pixels (positive when the stylus moves down relative to the touch-down
/// point). The touch state machine in process_touch() classifies a stylus
/// drag as "scroll" once vertical movement exceeds a threshold; the deltas
/// accumulate here. Callers (e.g. file-list windows) read this each frame
/// and forward it to ImGui::SetScrollY on the appropriate window/child.
